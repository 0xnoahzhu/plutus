...
-- group table is gone, `watchlist_items` no longer carries `watchlist_id`,
-- and `watchlist_reports` are global. The DROP/ALTER below clean up older
-- deployments that still have the per-group columns.
DROP TABLE IF EXISTS watchlists CASCADE;
ALTER TABLE watchlist_items DROP COLUMN IF EXISTS watchlist_id;
DROP INDEX IF EXISTS watchlist_items_watchlist_idx;
CREATE UNIQUE INDEX IF NOT EXISTS watchlist_items_stock_uniq
    ON watchlist_items (stock_id);

-- watchlist_reports: daily / weekly briefs for the watchlist. No group_id.
...
CREATE UNIQUE INDEX IF NOT EXISTS watchlist_reports_natural_key
    ON watchlist_reports (kind, period_start);
