...
pub async fn list(
    ...
    let mut body: Vec<MarketBriefOut> =
        page_slice.into_iter().map(Into::into).collect();
    let ids: Vec<i64> = body.iter().map(|b| b.id).collect();
    let read_ats =
        unread::read_ats(&state.db, user_id, EntityKind::MarketBrief, &ids).await?;
    for b in &mut body {
        b.read_at = read_ats.get(&b.id).map(jiff::Timestamp::to_string);
    }
    if p.is_paginating() {
...
pub async fn get(
    ...
    unread::mark_read(&state.db, user_id, EntityKind::MarketBrief, id).await?;
